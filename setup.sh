...
#
# Installs Docker and registers a GitHub Actions self-hosted runner.
# Environment variables (DB password, JWT secret, etc.) are stored as
# GitHub Secrets — the workflow writes .env fresh on every deploy.
#
# Usage (one-liner):
#   curl -fsSL https://raw.githubusercontent.com/altosecteam-org/AltoSec-Stress-Tester-Script/main/setup.sh \
#     | sudo bash -s -- --runner-token <TOKEN> [options]
#
# Required:
...
install_runner() {
    ...
    # Must run as root before config.sh — installs libicu/.NET Core 6.0 runtime deps
...
print_summary() {
    ...
    echo -e "\033[1;32m║        Altosec Stress Tester — Runner Setup Complete         ║\033[0m"
    echo -e "\033[1;32m╚══════════════════════════════════════════════════════════════╝\033[0m"
    echo ""
    ...
    echo ""
    echo "    SERVER_DOMAIN    → your server IP or domain"
    echo "    BACKEND_PORT     → API port       (e.g. 8080)"
    echo "    FRONTEND_PORT    → Frontend port  (e.g. 3000)"
    echo "    DB_PASSWORD      → strong random password"
    echo "    JWT_SECRET       → strong random string (min 32 chars)"
    echo ""
